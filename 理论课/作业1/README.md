...
## 作业总结
本次作业完成了基于OpenCV的YCbCr色彩空间图像处理实践，掌握了图像读取、色彩转换、下采样插值、PSNR评估、可视化展示等核心操作，理解了「亮度-色度分离」在图像压缩中的应用逻辑。后续可扩展方向包括：

1. 测试不同插值方法（最近邻、双三次）对PSNR值和重建质量的影响；
2. 调整下采样倍数（4×4、8×8），分析分辨率对图像质量的影响规律；
3. 基于YCbCr通道实现肤色检测、图像去噪等进阶处理；
4. 结合JPEG压缩原理，实现简易的图像压缩工具。
```

### 使用说明
1. 直接复制上述全部内容，粘贴到文本编辑器中，保存为`README.md`文件即可；
2. 所有格式（表格、代码块、列表、层级标题）完全匹配你提供的框架，无截断、无遗漏；
3. 内容100%贴合你的YCbCr处理代码，核心功能解析、问题排查等模块均对应代码逻辑；
4. 上传到GitHub/本地项目后，Markdown格式可正常渲染，代码块高亮、表格对齐。
